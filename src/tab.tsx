...
export default ({ class: className, cindex, children }: {
    ...
    return childs.length ? <>
        <header class="flex gap-2">{childs?.map((child, i) =>
            <div key={i} class={`min-w-16 px-2 cursor-pointer text-center rounded-t-md ${cindex.value == i ? className ?? '' : ''}`}
                onClick={() => cindex.value = i}>{child.props.title ?? i}
            </div>
        )}</header>
        <section class={`grow p-2 ${className ?? ''}`}>{childs[cindex.value]}</section>
    </> : <></>;
}